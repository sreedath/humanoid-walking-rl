"""
YouTube Shorts: Humanoid Learning to SPRINT
=============================================
Trains a humanoid to run as fast as possible using high forward velocity reward.
Shows progression from stumbling to full sprint.
Uses Brax PPO with GPU-accelerated physics.
"""
...
# ---------------------------------------------------------------------------
# Sprint Environment: reward maximum forward velocity
# ---------------------------------------------------------------------------
class SprintEnv(PipelineEnv):
    """Humanoid trained for maximum forward velocity (sprinting)."""

    def __init__(self, backend: str = "generalized", **kwargs):
        env = envs.get_environment(env_name="humanoid", backend=backend)
        self._base_env = env
        self.sys = env.sys
        super().__init__(sys=env.sys, backend=backend, n_frames=5)

    def reset(self, rng: jax.Array) -> State:
        return self._base_env.reset(rng)

    def step(self, state: State, action: jax.Array) -> State:
        """Override reward: heavily weight forward velocity."""
        next_state = self._base_env.step(state, action)

        # Get COM velocity (forward = x direction)
        # Extract from pipeline state
        pipeline_state = next_state.pipeline_state
        torso_vel = pipeline_state.xd.vel[0]  # torso velocity
        forward_vel = torso_vel[0]  # x-velocity

        # Height for survival check
        torso_height = pipeline_state.x.pos[0][2]
        healthy = jnp.where(
            jnp.logical_and(torso_height > 0.8, torso_height < 2.5),
            jnp.float32(1.0), jnp.float32(0.0))

        # Sprint reward: heavily reward forward velocity
        speed_reward = forward_vel * 3.0  # 3x weight on speed
        healthy_reward = healthy * 2.0
        ctrl_cost = -0.05 * jnp.sum(action ** 2)

        reward = speed_reward + healthy_reward + ctrl_cost

        # Done if fallen
        done = jnp.where(healthy > 0.5, jnp.float32(0.0), jnp.float32(1.0))
...
def main():
    ...
    print("=" * 60)
    print("  Humanoid Sprint - GPU-Accelerated RL")
    print("=" * 60)
    print(f"  GPU: {jax.devices()[0]}")

    # Use standard humanoid but with sprint rewards
    # We train using the standard humanoid env then override reward at eval
    ...
    # Train with higher reward_scaling to encourage faster learning
    ...
    brax_model.save_params(os.path.join(config.output_dir, "sprint_params"), final_params)

    # Select narrative
    n = len(checkpoints)
    narrative = [
        (checkpoints[0],          "First Steps",       180),
        (checkpoints[max(1,n//8)], "Learning Balance",  180),
        (checkpoints[n//4],       "Finding Speed",     240),
        (checkpoints[n//2],       "Picking Up Pace",   300),
        (checkpoints[3*n//4],     "Almost There",      300),
        (checkpoints[-1],         "Full Sprint",       900),
    ...
    # Compile video
    output_path = os.path.join(config.output_dir, "sprint_shorts.mp4")
    ...
    for f in make_title_card(
        ["Humanoid Learning", "to SPRINT", "", "Brax PPO | GPU Physics"],
    ...
    for f in make_transition(0.3, config):
        writer.append_data(f); total_frames += 1
    for f in make_title_card(
        [f"Trained in {train_time:.0f}s", f"Final Reward: {checkpoints[-1].reward:.0f}",
         "", "50M steps | A100 GPU"],
    ...
    print(f"\n{'='*60}")
    print(f"  Sprint Shorts Complete")
