"""
YouTube Shorts: One-Legged Hopper Learning to Bounce
=====================================================
A single-legged pogo-stick creature learning to hop forward.
Classic RL env — goofy and satisfying to watch.
"""
...
def main():
    ...
    print("=" * 60)
    print("  Hopper Learning to Bounce - GPU RL")
    print("=" * 60)
    print(f"  GPU: {jax.devices()[0]}")

    env = envs.get_environment(env_name="hopper", backend="generalized")

    checkpoints = []; progress_metrics = {}; t0 = time.time()

    def progress_fn(num_steps, metrics):
        r = float(metrics.get("eval/episode_reward", 0))
        progress_metrics[num_steps] = r
        print(f"  step={num_steps:>12,}  reward={r:>8.1f}  time={time.time()-t0:.0f}s")

    def policy_params_fn(current_step, make_policy, params):
        checkpoints.append(Checkpoint(step=current_step, reward=0.0, params=jax.device_get(params)))

    train_fn = functools.partial(
        ppo.train, num_timesteps=config.num_timesteps, num_evals=config.num_evals,
        reward_scaling=30, episode_length=config.episode_length,
        normalize_observations=True, action_repeat=1, unroll_length=5,
        num_minibatches=32, num_updates_per_batch=4, discounting=0.97,
        learning_rate=3e-4, entropy_cost=1e-2, num_envs=config.num_envs,
        batch_size=2048, seed=3,
    )

    print("\nTraining...\n")
    make_inference_fn, final_params, _ = train_fn(
        environment=env, progress_fn=progress_fn, policy_params_fn=policy_params_fn,
    )
    ...
    brax_model.save_params(os.path.join(config.output_dir, "hopper_params"), final_params)

    n = len(checkpoints)
    narrative = [
        (checkpoints[0],          "First Bounce",      180),
        (checkpoints[max(1,n//8)], "Wobble Wobble",     180),
        (checkpoints[n//4],       "Getting Air",       240),
        (checkpoints[n//2],       "Hop Hop Hop",       300),
        (checkpoints[3*n//4],     "Bouncing Along",    300),
        (checkpoints[-1],         "Pogo Master",       900),
    ] if n >= 6 else [(c, f"Step {c.step:,}", 300) for c in checkpoints]

    mj_model = env.sys.mj_model; mj_data = mujoco.MjData(mj_model)
    ...
    output_path = os.path.join(config.output_dir, "hopper_shorts.mp4")
    writer = imageio.get_writer(
        output_path, fps=config.fps, codec="libx264", quality=9,
    ...
    for f in make_title_card(
        ["One-Legged Hopper", "Learning to BOUNCE", "", "Brax PPO | GPU Physics"],
        config, dur=3.0, font_sizes=[48, 52, 20, 28],
    ):
        writer.append_data(f); total_frames += 1

    for seg_idx, (ckpt, label, max_steps) in enumerate(narrative):
        print(f"\n--- {label} ---")
        for f in make_transition(0.3, config): writer.append_data(f); total_frames += 1

        inference_fn = make_inference_fn(ckpt.params)
        jit_infer = jax.jit(inference_fn)
        jit_reset = jax.jit(env.reset); jit_step = jax.jit(env.step)
        ...
        while len(pipeline_states) < 60:
            pipeline_states.append(pipeline_states[-1]); positions.append(positions[-1])
        ...
        for i, pstate in enumerate(pipeline_states):
            ...
            cam.lookat[0] = positions[i][0]; cam.lookat[1] = positions[i][1]
            cam.lookat[2] = config.cam_lookat_z
            cam.distance = config.cam_distance; cam.elevation = config.cam_elevation
            ...
            if (i+1) % 5 == 0: writer.append_data(comp); fc += 1
        total_frames += fc
    ...
    for f in make_transition(0.3, config): writer.append_data(f); total_frames += 1
    for f in make_title_card(
        [f"Trained in {train_time:.0f}s", f"Reward: {checkpoints[-1].reward:.0f}", "", "30M steps | A100"],
        config, dur=2.5, font_sizes=[40, 44, 20, 28],
    ):
        writer.append_data(f); total_frames += 1
    for f in make_title_card(["GPU-Accelerated RL", "", "google/brax"],
        config, dur=2.0, font_sizes=[42, 20, 28],
    ):
        writer.append_data(f); total_frames += 1

    writer.close()
    dur = total_frames / config.fps
    sz = os.path.getsize(output_path) / (1024**2)
    print(f"\n{'='*60}\n  Hopper Shorts Complete\n  {output_path}\n  {dur:.1f}s | {sz:.1f} MB\n{'='*60}")
